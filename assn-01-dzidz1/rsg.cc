...
void generate(string& nonTerminal, map<string, Definition>& grammar) 
{
  map<string, Definition>::iterator found = grammar.find(nonTerminal);
  if(found != grammar.end()) {
    Definition definition = found->second;
    Production production = definition.getRandomProduction();
    for (Production::iterator curr = production.begin(); curr != production.end(); ++curr) {
      if(grammar.find(*curr) != grammar.end()) {
        generate(*curr, grammar);
      } else {
        cout << *curr << " ";
      }
    }
