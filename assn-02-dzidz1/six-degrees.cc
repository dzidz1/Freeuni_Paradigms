...
void generateShortestPath(string source, string target, const imdb& db) {
  ...
  while(!partialPaths.empty() && partialPaths.front().getLength() <= 5) {
      ...
      vector<film> last_actor_films;
      bool tmp1 = db.getCredits(front_path.getLastPlayer(), last_actor_films);
      if (!tmp1) continue;

      for(film movie : last_actor_films) {
          if(previouslySeenFilms.insert(movie).second) {
              vector<string> movie_cast;
              bool tmp2 = db.getCast(movie, movie_cast);
              if(!tmp2) continue;

              for(string costar : movie_cast) {
                if(previouslySeenActors.insert(costar).second) {
                  path clone = front_path;
                  clone.addConnection(movie, costar);
