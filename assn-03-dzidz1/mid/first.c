...
void CountingArrayInit(CountingArray* a, int elem_size, CmpFn cmp_fn, FreeFn free_fn) {
	...
	a->all_together = 0;
	a->alloc_len = 4;
	a->root_uniques = malloc(a->elem_size * a->alloc_len);
	a->root_all = malloc(a->elem_size * a->alloc_len);
	assert(a != NULL);
	a->CmpFn = cmp_fn;
	a->FreeFn = free_fn;
}
...
bool CountingArrayInsert(CountingArray* a, void* elem) {
	if (a->log_len == a->alloc_len) {
		a->alloc_len *= 2;
		a->root_uniques = realloc(a->alloc_len * a->elem_size);
		assert(a->root_uniques != NULL);
	}

	if (a->all_together == a->alloc_len) {
		a->alloc_len *= 2;
		a->root_all = realloc(a->alloc_len * a->elem_size);
	...
	if (contains == false) {
		...
		if (delta > 0) {
			memmove(old_ptr + a->elem_size, old, a->elem_size * delta);
			memcpy(old_ptr, elem);
		}
		else {
			memcpy((void*)((char*)a->root_uniques + a->elem_size * a->log_len), elem);
		}
		a->log_len++;
	}
	else {
		...
		int delta = a->log_len - i;
		if (delta > 0) {
			memmove((void*)((char*)old_ptr + a->elem_size), old, a->elem_size * delta);
			memcpy(old_ptr, elem);
		}
		else {
			memcpy((void*)((char*)a->root_uniques + a->elem_size * a->all_together), elem);
		}
	}
	a->all_together++;
...
void CountingArrayRemove(CountingArray* a, void* elem) {
	...
	if (contains) {
		...
		if (delta > 0) {
			memmove((char*)a->root_uniques + (i - 1) * a->elem_size, (void*)((char*)should_remove + a->elem_size), delta * a->elem_size);
		}
		else {
		...
		if (delta > 0) {
			memmove((void*)((char*)a->root_all + (i - 1) * a->elem_size), (void*)((char*)should_remove + a->elem_size), delta * a->elem_size);
		}
		else {
