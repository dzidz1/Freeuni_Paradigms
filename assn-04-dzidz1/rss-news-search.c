...
static void BuildIndices(const char *feedsFileName, hashset *s,
hashset *u, hashset *i);
static void ProcessFeed(const char *remoteDocumentName, hashset *stop_words_set, 
hashset *seen_articles_urls, hashset *indexes);
static void PullAllNewsItems(FILE *dataStream, hashset *stop_words_set,
hashset *seen_articles_urls, hashset *indexes);
static bool GetNextItemTag(streamtokenizer *st);
static void ProcessSingleNewsItem(streamtokenizer *st, hashset *stop_words_set,
hashset *seen_articles_urls, hashset *indexes);
...
static void ParseArticle(const char *articleTitle,
                         ...
                         const char *articleURL, hashset *stop_words_set, 
                         hashset *seen_articles_urls, 
                         hashset *indexes);
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
                        const char *unused, const char *articleURL,
                        hashset *stop_words_set,
                        hashset *seen_articles_urls,
                        hashset *indexes);
static void QueryIndices(const hashset *stop_words_set, hashset *indexes);
static void ProcessResponse(const char *word, hashset *indexes, const hashset *stop_words_set);
static bool WordIsWellFormed(const char *word);

void build_stop_words_set(hashset *set);
...
static const int kNumArticlesOnScreen = 10;
static const int sec_num_buckets = 1007;

static const signed long kHashMultiplier = -1664117991L;
static int str_hash(const void *str, int num_buckets)  
{            
  int i;
...
void str_free(void *elem_ptr) {
  char *s = *(char **)elem_ptr;
  free(s);
}

int str_cmp(const void *elem_ptr1, const void *elem_ptr2) {
  const char *str1 = *(const char **)elem_ptr1;
  const char *str2 = *(const char **)elem_ptr2;
...
int freq_cmp(const void *elem_ptr1, const void *elem_ptr2) {
  freq *f1 = (freq *)elem_ptr1;
  freq *f2 = (freq *)elem_ptr2;

  //if (f1->key == NULL || f2->key == NULL) return -1;

  return str_cmp(&(f1->key), &(f2->key));
}

int freq_hash(const void *elem_ptr, int num_buckets) {
  freq *f = (freq *)elem_ptr;
  return str_hash(&(f->key), num_buckets);
}

void freq_free(void *elem_ptr) {
  freq *f = (freq *)elem_ptr;
  if (f->key != NULL) free(f->key);
}


int art_cmp(const void *elem_ptr1, const void *elem_ptr2) {
  int i = (*(article *)elem_ptr1).freq;
  int j = (*(article *)elem_ptr2).freq;
  return i - j;
}

void art_free(void *elem_ptr) {
  article *a = (article *)elem_ptr;
  free(a->art_url);
  free(a->art_title);
}

void art_sort(void *elem_ptr, void *aux) {
  idx *i = (idx *)elem_ptr;
  vector *vec = (vector *)i->articles;
  VectorSort(vec, art_cmp);
}


int idx_cmp(const void *elem_ptr1, const void *elem_ptr2) {
  idx *i = (idx *)elem_ptr1;
  idx *j = (idx *)elem_ptr2;
  return str_cmp(&(i->key), &(j->key));
}

int idx_hash(const void *elem_ptr, int num_buckets) {
  idx *i = (idx *)elem_ptr;
  return str_hash(&(i->key), num_buckets);
}

void idx_free(void *elem_ptr) {
  idx *i = (idx *)elem_ptr;
  free(i->key);
  VectorDispose(i->articles);
  free(i->articles);
}
...
int main(int argc, char **argv) {
  ...
  hashset *stop_words_set = malloc(sizeof(hashset));
  HashSetNew(stop_words_set, sizeof(char *), num_buckets, str_hash, str_cmp, str_free);
  build_stop_words_set(stop_words_set);

  hashset *seen_article_urls = malloc(sizeof(hashset));
  HashSetNew(seen_article_urls, sizeof(char *), num_buckets, str_hash, str_cmp, str_free);

  hashset *indexes = malloc(sizeof(hashset));
  HashSetNew(indexes, sizeof(idx), sec_num_buckets, idx_hash, idx_cmp, idx_free);


  BuildIndices((argc == 1) ? kDefaultFeedsFile : argv[1], stop_words_set, seen_article_urls, indexes);
  
  HashSetMap(indexes, art_sort, NULL);
  QueryIndices(stop_words_set, indexes);
  curl_global_cleanup();

  HashSetDispose(stop_words_set);
  HashSetDispose(seen_article_urls);
  HashSetDispose(indexes);
  free(stop_words_set);
  free(seen_article_urls);
  free(indexes);
  return 0;
}
...
void build_stop_words_set(hashset *set) {
  ...
  STNew(&tokenizer, infile, kNewLineDelimiters, true);
  while(STNextToken(&tokenizer, buff, sizeof(buff))) {
    char *str = strdup(buff);
    HashSetEnter(set, &str);
  }
...
static void BuildIndices(const char *feedsFileName, hashset *stop_words_set,
hashset *seen_articles_urls, hashset *indexes) {
  FILE *infile;
  streamtokenizer st;
  ...
  while (STSkipUntil(&st, ":") !=
    ...
    ProcessFeed(remoteFileName, stop_words_set, seen_articles_urls, indexes);
  }
...
static void ProcessFeedFromFile(char *fileName, hashset *stop_words_set,
hashset *seen_articles_urls, hashset *indexes) {
  FILE *infile;
  streamtokenizer st;
  ...
  ScanArticle(&st, (const char *)fileName, articleDescription,
              (const char *)fileName, stop_words_set, seen_articles_urls, indexes);
  STDispose(&st); // remember that STDispose doesn't close the file, since STNew
                  // doesn't open one..
  fclose(infile);
}
...
hashset *seen_articles_urls, hashset *indexes) {
  if (!strncmp(kFilePrefix, remoteDocumentName, strlen(kFilePrefix))) {
    ProcessFeedFromFile((char *)remoteDocumentName + strlen(kFilePrefix), stop_words_set, seen_articles_urls, indexes);
    return;
  }

  FILE *tmpFeed = FetchURL(remoteDocumentName, "tmp_feed");
  PullAllNewsItems(tmpFeed, stop_words_set, seen_articles_urls, indexes);
  fclose(tmpFeed);
}
...
hashset *seen_articles_urls, hashset *indexes) {
  ...
  while (GetNextItemTag(
    ...
    ProcessSingleNewsItem(&st, stop_words_set, seen_articles_urls, indexes);
  }
...
hashset *seen_articles_urls, hashset *indexes) {
  ...
  ParseArticle(articleTitle, articleDescription, articleURL, stop_words_set, seen_articles_urls, indexes);
}
...
static void ParseArticle(const char *articleTitle,
  ...
  ScanArticle(&st, articleTitle, articleDescription, articleURL, stop_words_set, seen_articles_urls, indexes);
  STDispose(&st);
  fclose(tmpDoc);
}


void freq_idx_map(void *elem_ptr, void *aux_data) {
  freq *f = (freq *)elem_ptr;
  aux *a = (aux *)aux_data;
  ...
  article ar;
  ar.art_url = strdup(a->art_url);
  ar.art_title = strdup(a->art_title);
  ar.freq = f->val;

  idx *found = (idx *)(HashSetLookup(a->indexes, &i));
  if(found == NULL) {
    idx j;
    j.key = strdup(f->key);
    j.articles = malloc(sizeof(vector));
    if(j.articles == NULL) {
      free(j.key);
      return;
    }

    VectorNew(j.articles, sizeof(article), art_free, 0);
    VectorAppend(j.articles, &ar);
    HashSetEnter(a->indexes, &j);
  } else {
    VectorAppend(found->articles, &ar);
  }
}
...
static void ScanArticle(streamtokenizer *st, const char *articleTitle,
  ...
  hashset *frequencies_set = malloc(sizeof(hashset));
  HashSetNew(frequencies_set, sizeof(freq), num_buckets, freq_hash, freq_cmp, freq_free);
  ...
  while (STNextToken(st, word, sizeof(word))) {
    if (strcasecmp(word, "<") == 0) {
      ...
      char *pw = word;
      to_lower_case(pw);
      if (WordIsWellFormed(pw) && HashSetLookup(stop_words_set, &pw) == NULL) {
        freq f;
        f.key = pw;
        f.val = 0;

        freq *found = (freq *)(HashSetLookup(frequencies_set, &f));
        if(found == NULL) {
          f.key = strdup(pw);
          f.val = 1;
          HashSetEnter(frequencies_set, &f);
        } else {
          (found->val)++;
        }
  ...
  aux a;
  a.art_url = strdup(articleURL);
  a.art_title = strdup(articleTitle);
  a.indexes = indexes;

  HashSetMap(frequencies_set, freq_idx_map, &a);
  
  free(a.art_url);
  free(a.art_title);

  HashSetDispose(frequencies_set);
  free(frequencies_set);
 ...
 * then proceeds (via ProcessResponse)s to list up to 10 articles (sorted by
 * relevance) that contain that word.
 */

static void QueryIndices(const hashset *stop_words_set, 
  hashset *indexes) {
  char response[1024];
  while (true) {
    ...
    if (strcasecmp(response, "") == 0)
      break;
    ProcessResponse(response, indexes, stop_words_set);
  }
}
...
static void ProcessResponse(const char *word, hashset *indexes, const hashset* stop_words_set) {
  ...
  to_lower_case(tmp);
  if(HashSetLookup(stop_words_set, &tmp) != NULL) {
  ...
  if (WordIsWellFormed(word)) {
    ...
    idx *found = (idx *)HashSetLookup(indexes, &i);
    if(found == NULL) {
      printf("None of today's news articles contain the word \"%s\".\n\n", word);
    } else {
      //printf("Nice! We found %d articles that include the word \"%s\".\n\n", VectorLength(found->articles), word);
      int n;
      if(VectorLength(found->articles) >= kNumArticlesOnScreen) {
        n = kNumArticlesOnScreen;
      } else {
        n = VectorLength(found->articles);
      }

      int size;
      size = VectorLength(found->articles);
      int cnt;
      cnt = 1;
      for(int in = size - 1; in >= size - n; in--) {
        article *x = (article *)VectorNth(found->articles, in);
        if(x->freq != 1) {
          printf("%d.) \"%s\" [search term occurs %d times]\n", cnt, x->art_title, x->freq);
        } else {
