...
(define (half list)
  (let loop((ls '()) (n (div (length list) 2)) (seq list))
    (if (= n 0) ls
      (loop (cons (car seq) ls) (- n 1) (cdr seq))
    )
  )
)
...
(define (shuffle lst)
  (let loop((ans '()) (lst1 (half lst)) (lst2 (half (reverse lst))))
    (if(null? lst1)ans
      (loop (cons (car lst1)(cons (car lst2) ans))) 
    )
  )
)
