...
(define (single_intersection circle lst)
...
(define (intersection-points lst)
  (if (null? lst)
    '()
    (append (single_intersection (car lst) (cdr lst)) (intersection-points (cdr lst)))
  )
)


(define (distance-product pt lst)
  (apply * (map (lambda (x) (dist pt x)) (remove pt lst)))
)


(define (rate-points lst)
  (map (lambda (x) (list (distance-product x (remove x lst)) x)) lst)
)


(define (sort-points lst)
  (quicksort lst (lambda (x y) (< (car x) (car y))))
)



(define (clumped-points lst)
  (apply append (map (lambda (x) (cdr x)) (prefix-of-list (sort-points (rate-points lst)) (div (length lst) 2))))
)

(define (sumx lst)
  (if (null? lst)
    0
    (+ (car (car lst)) (sumx (cdr lst)))
  )
)
...
(define (sumy lst)
  (if (null? lst)
    0
    (+ (cadr (car lst)) (sumy (cdr lst)))
  )
)


(define (average-point lst)
  (list (distance-product (list (/ (sumx lst) (length lst)) (/ (sumy lst) (length lst))) lst)
        (list (/ (sumx lst) (length lst)) (/ (sumy lst) (length lst)))
  )
)
...
(define (where-am-i dstlst strlst)
  (sort-points (map best-estimate (all-guesses dstlst strlst)))
)
